...
**Fastest way — one paste:** select everything in the starter `Code.gs`, delete it, and
paste the whole of [`ALL-IN-ONE.gs`](./ALL-IN-ONE.gs) instead. That file is every other
file concatenated, so one paste is the entire system. Save, and skip to Step 3.

*(It is generated by `build-system-prompt.py` and verified against the full test suite, so
it behaves identically to the six separate files. Six pastes is the step most likely to go
wrong, because a missed file fails at runtime rather than visibly.)*

Prefer to read the code in pieces? Then six files:
