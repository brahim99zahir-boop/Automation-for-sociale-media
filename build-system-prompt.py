...
def main() -> None:
    ...
    # SystemPrompt.gs must exist before the bundle is assembled.
    build_bundle()
...
def build_bundle():
    """
    Concatenate every .gs into one file.

    Apps Script puts all files in one global scope, so a single paste behaves identically
    to six — and six separate copy-pastes is the step most likely to go wrong, since one
    missed file fails at runtime rather than visibly. Order matters only because Config
    defines CONFIG and PROP, which Settings reads at load.
    """
    order = ["Config.gs", "SystemPrompt.gs", "Settings.gs",
             "Platforms.gs", "Code.gs", "Dashboard.gs"]
    src = ROOT / "4-apps-script"
    parts = [
        "/**\n"
    ...
    out = src / "ALL-IN-ONE.gs"
    out.write_text("\n".join(parts), encoding="utf-8")
    print(f"{'ALL-IN-ONE.gs':<24}{len(out.read_text(encoding='utf-8')):,} chars")
